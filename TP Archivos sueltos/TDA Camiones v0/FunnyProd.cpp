...
FILE *ptr1;
    ...
    if((ptr1=fopen("ingresos.db","r"))==NULL){
         ...
         int pos=ftell(ptr1);
printf("%d posicion inicial",pos);

    fscanf (ptr1, "%s", cadenaI);
...
FILE *ptr2;
 ...
 if((ptr2=fopen("solicitudes.db","r"))==NULL){
        ...
        int pos2=ftell(ptr2);
printf("%d posicion inicial",pos2);
 fscanf (ptr2, "%s", cadenaS) ;
...
//reponer articulo
        while (caracter != '<'){
        fseek( ptr2, -2, SEEK_CUR );
        fread (&caracter, sizeof(caracter), 1, ptr2);
        }
        pos2=ftell(ptr2);
    printf("%d posicion final",pos2);
    }

    else{
        printf("entro en else");
        while (caracter != '<'){
        fseek( ptr1, -2, SEEK_CUR );
        fread (&caracter, sizeof(caracter), 1, ptr1);

        }
        pos=ftell(ptr1);
    printf("%d posicion final",pos);
//despachar pedido(restar del stok)
//si no se puede poner en faltantes
//volver linea
    }
fclose (ptr2); // cierra el archivo
