     ...
     while (( cursor != fin( )) && (!encontrado)){
        ...
        if (!encontrado){
         ...
         PtrNodoLista nodo=adicionarPrincipio(lstCandidatos, candidato);
          ...
          printf("Ingrese el ID del partido que desea que tenga el candidato\n");
          scanf("%d",&idPartido);
        ...
        scanf("%c", &rta);
         ...
         system("CLS");
         printf("En instantes sera redirigido a la pantalla principal");
...
void bajaCandidato(Lista &lstCandidatos){
    ...
    printf("Desea seguir eliminando candidatos? S/N\n");
    fflush(stdin);
    scanf("%c", &rta);
    fflush(stdin);
    }
